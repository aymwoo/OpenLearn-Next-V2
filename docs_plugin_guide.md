...
export default function ClassroomQuizWidget({ context }: { context: any }) {
  ...
  useEffect(() => {
    ...
    const handleAnswerCast = async (event: any) => {
      if (
        event.type === 'ext-classroom-quiz.answer_submitted' &&
        activeQuiz &&
        event.payload.questionId === activeQuiz.quizId
      ) {
        await refreshStats();
      }
    };
  ...
  return (
    <div style={{ padding: 20, background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0' }}>
      ...
      {!activeQuiz ? (
        ...
        <div>
          <div style={{ fontWeight: 'bold', fontSize: 16, marginBottom: 12 }}>Q: {activeQuiz.question}</div>
