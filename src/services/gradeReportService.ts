...
export async function generateClassPDFReport(options: GeneratePDFReportOptions) {
  ...
  try {
    ...
    const classSubmissionRate =
      totalClassOpportunities > 0 ? (totalClassSubmissions / totalClassOpportunities) * 100 : 0;
    ...
    doc.text('Generated by OpenLearn Next • Educational Operating System', 14, 282);
...
export function exportClassGradesCSV(options: {
  ...
  const { className, students, dashData, qWeight = 40, aWeight = 60, overrides = {} } = options;
...
export function exportAllClassesCombinedCSV(options: {
    ...
    cStudents.forEach((st: any) => {
      ...
      classifiedAssignments.forEach((a: any) => {
        const scoreObj = dashData.performance?.find(
          (p: any) =>
            p.student_id === st.id && p.assignment_id === a.id && p.submission_status === 'graded' && p.score !== null,
        );
        if (scoreObj) {
  ...
  const csvContent = 'data:text/csv;charset=utf-8,\uFEFF' + csvRows.map((e) => e.map(escapeCSV).join(',')).join('\n');
