...
export function StudentGradedTimeline({ assignments }: StudentGradedTimelineProps) {
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-8 hover:shadow-md transition-all">
        <div className="max-w-md mx-auto text-center py-6">
          ...
          <p className="text-gray-500 text-xs mt-2 leading-relaxed">
            Your interactive learning timeline and performance chart will generate automatically here once teachers
            grade your MCQ quizzes or interactive submissions. Complete more tasks to build your historical growth
            profile!
          </p>
        </div>
      </div>
  ...
  const chartData = gradedList.map((item, idx) => ({
    ...
    dateStr: item.submitted_at
      ? new Date(item.submitted_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })
      : `Quiz #${idx + 1}`,
    fullDate: item.submitted_at
      ? new Date(item.submitted_at).toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })
      : 'N/A',
  ...
  const getScoreColor = (score: number) => {
    if (score >= 90)
      return { bg: 'bg-green-50 text-green-700 border-green-200', text: 'text-green-600', ring: 'ring-green-400' };
    if (score >= 75)
      return { bg: 'bg-blue-50 text-blue-700 border-blue-200', text: 'text-blue-600', ring: 'ring-blue-400' };
    if (score >= 60)
      return { bg: 'bg-amber-50 text-amber-700 border-amber-200', text: 'text-amber-600', ring: 'ring-amber-400' };
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden flex flex-col font-sans">
        ...
        <div className="flex items-center gap-1.5 bg-white p-1 rounded-lg border border-gray-200 self-start sm:self-center">
          <button
            ...
            className={`px-2.5 py-1 text-xs font-semibold rounded-md transition-all flex items-center gap-1 ${
              chartType === 'area' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
            }`}
          >
          ...
          <button
            ...
            className={`px-2.5 py-1 text-xs font-semibold rounded-md transition-all flex items-center gap-1 ${
              chartType === 'bar' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
            }`}
          >
            <BarChart2 size={12} />
            <span>Bars</span>
      ...
      <div className="p-4 sm:p-6 space-y-6">
        ...
        {scores.length >= 2 && (
          <div
            className={`p-4 rounded-xl border flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all ${trajectoryColor}`}
          >
            <div className="flex items-start gap-3">
              <div className="p-2 rounded-lg bg-white/95 text-indigo-650 shrink-0 border border-indigo-100 shadow-xs">
                <TrendingUp
                  size={18}
                  className={trajectoryDelta > 0 ? 'text-emerald-600 animate-pulse' : 'text-indigo-600'}
                />
              </div>
              <div>
            ...
            <div className="text-[11px] font-semibold px-3 py-2 bg-white/95 rounded-lg shadow-3xs border border-slate-100 text-slate-700 md:self-center">
              ...
              <span className="font-bold text-emerald-700">
                {Math.round(
                  scores.slice(Math.ceil(scores.length / 2)).reduce((a, b) => a + b, 0) /
                    scores.slice(Math.ceil(scores.length / 2)).length,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0">
          ...
          <div className="lg:col-span-8 flex flex-col justify-between">
            <div className="mb-2 text-xs font-semibold uppercase text-gray-400 tracking-wider">
              Chronological Progress Chart
            </div>
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="space-y-0.5">
                          ...
                          <h4 className="font-bold text-xs text-gray-800 leading-snug line-clamp-1 py-0.5">
                            {item.fullTitle}
                          </h4>
                        </div>
                        <div
