                ...
                <div className="p-3.5 bg-indigo-50 border border-indigo-100 rounded-xl text-xs leading-relaxed">
                  ...
                  <div className="text-gray-650 space-y-1">
                    <div>
                      •{' '}
                      {lang === 'zh'
                        ? `本周起始日期（周一）：${mondayDate}`
                        : `Week Start Date (Monday): ${mondayDate}`}
                    </div>
                    <div>
                      •{' '}
                      {lang === 'zh'
                        ? '班级匹配：系统将根据识别到的班级名称自动导入至系统中对应班级。若班级不存在，将自动创建。'
                        : 'Class Matching: System will automatically import entries into matched classes or create new classes on the fly.'}
                    </div>
                  </div>
                </div>
          ...
          >
            {ocrImporting ? (
              ...
              <>
                <Check size={14} />
                {lang === 'zh'
                  ? `确认导入选中的 ${ocrSelectedEntries.size} 条课程`
                  : `Import ${ocrSelectedEntries.size} Selected Entries`}
              </>
            )}
          </button>
        </div>
