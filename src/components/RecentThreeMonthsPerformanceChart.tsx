...
export function RecentThreeMonthsPerformanceChart({
  assignments = [],
  lang = 'en',
}: RecentThreeMonthsPerformanceChartProps) {
  ...
  const { filteredAssignments, dateRangeLabel, stats } = useMemo(() => {
    ...
    const filtered =
      filterMode === '3months' ? afterClassFilter.filter((ast) => ast.actionTime >= limitTime) : afterClassFilter;

    // Standard date strings
    const startDateStr = new Date(limitTime).toLocaleDateString(lang === 'zh' ? 'zh-CN' : 'en-US', {
      month: 'short',
      day: 'numeric',
      year: 'numeric',
    });
    const endDateStr = new Date(anchorTime).toLocaleDateString(lang === 'zh' ? 'zh-CN' : 'en-US', {
      month: 'short',
      day: 'numeric',
      year: 'numeric',
    });
  ...
  return (
    <div
      ...
      id="three-months-performance-line-chart"
    >
      {/* Chart Header */}
        ...
        {filteredAssignments.length === 0 ? (
          ...
          <div className="h-[240px] w-full relative">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={filteredAssignments} margin={{ top: 15, right: 15, left: -22, bottom: 5 }}>
                <defs>
