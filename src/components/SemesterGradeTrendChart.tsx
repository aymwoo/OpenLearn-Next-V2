...
export function SemesterGradeTrendChart({ assignments = [], lang = 'en' }: SemesterGradeTrendChartProps) {
  ...
  const t = {
    ...
    subtitle:
      lang === 'zh'
        ? '全学期已评分测验与交互式作业的 chronological 成绩趋势分析'
        : 'Chronological grade analysis of graded quizzes and assignments across the semester',
    ...
    emptyDesc:
      lang === 'zh'
        ? '当前学期尚未有已评分的测验。当物理、历史或数学老师评估并评定你的白板作业或选择题测试后，由于底层的自适应评估引擎同步，此处会自动渲染出成绩波动趋势。'
        : 'You do not have any graded assignments or quizzes yet in this semester. Once teachers complete grading your interactive assignments, your chronological performance trajectory line will outline your academic development.',
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-2xl p-8 shadow-sm hover:shadow-md transition-all">
        <div className="max-w-md mx-auto text-center py-8">
          ...
          <p className="text-gray-500 text-xs mt-3 leading-relaxed">{t.emptyDesc}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      className="bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col font-sans"
      id="semester-grade-trend-container"
    >
        ...
        <div className="lg:col-span-9 flex flex-col justify-between" id="semester-grade-chart-wrapper">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
            <div className="text-xs font-bold text-indigo-900 flex items-center gap-1">
              ...
              <span>
                {lang === 'zh' ? '学期表现时间轴（按提交时间排序）' : 'Academic Performance Timeline (Chronological)'}
              </span>
            </div>
                  ...
                  .map((d, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`px-2 py-1 rounded-lg border font-mono font-bold text-center text-xs shrink-0 shadow-3xs bg-white ${
                            d.score >= 85
                              ? 'text-emerald-700 border-emerald-150'
                              : d.score >= 60
                                ? 'text-indigo-700 border-indigo-150'
                                : 'text-rose-700 border-rose-150'
                          }`}
                        >
                          {d.score}%
                        </div>
              ...
              <div className="mt-4 p-3 bg-gradient-to-r from-indigo-500/10 to-purple-500/10 rounded-2xl border border-indigo-100/50 text-xs text-slate-700 leading-relaxed font-medium">
                {lang === 'zh' ? (
                  <>
                    ...
                    次正式学习质量评测。全科平均通过率为{' '}
                    <span className="font-bold text-emerald-700 font-mono">{stats.passRate}%</span>
                    。建议继续针对得分低于 60% 的章节进行自主微课回顾和重新作答。
                  </>
                ) : (
                  <>
                    You participated in <span className="font-bold text-indigo-805 font-mono">{stats.total}</span>{' '}
                    scholastic assessments this semester with an average competency score of{' '}
                    <span className="font-bold text-indigo-700 font-mono">{stats.average}%</span>. Your grade profile
                    shows an active rate of <span className="font-bold text-teal-700 font-mono">{stats.passRate}%</span>{' '}
                    above passing grade threshold.
                  </>
                )}
              </div>
            )}
