...
export function AcademicGrowthTrajectoryChart({ assignments = [], lang = 'en' }: AcademicGrowthTrajectoryChartProps) {
  ...
  const { trajectoryData, availableClasses, stats, trendDirection } = useMemo(() => {
    ...
    const filtered =
      selectedClass === 'all' ? withScores : withScores.filter((ast) => ast.class_name === selectedClass);
    ...
    const computedTrajectory = filtered.map((ast, idx) => {
      ...
      const formattedDate = dateObj.toLocaleDateString(lang === 'zh' ? 'zh-CN' : 'en-US', {
        month: 'short',
        day: 'numeric',
      });
  ...
  const t = {
    ...
    subtitle:
      lang === 'zh'
        ? '追踪在校作业评分的累积移动平均线，体现核心学力成长趋势'
        : 'Tracks cumulative moving average of evaluated coursework to outline long-term proficiency',
    ...
    emptyDesc:
      lang === 'zh'
        ? '当你的作业通过评阅且录入成绩后，成长轨迹分析引擎将自动计算每次评分后的全局累积分，从而绘制出能体现学习毅力与知识掌握成熟度的进展曲线。'
        : 'Once your work receives evaluations, our trajectory motor calculates a progressive running average to render a true representation of your academic journey.',
  ...
  return (
    <div
      ...
      id="academic-growth-trajectory-card"
    >
      {/* Upper header */}
      <div className="p-5 border-b border-gray-100 bg-linear-to-r from-emerald-50/10 via-white to-teal-50/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 self-center">
          ...
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${showIndividualScores ? 'bg-emerald-500 animate-pulse' : 'bg-gray-300'}`}
            />
            {t.layerIndividual}
          </button>
        </div>
              ...
              <div className="p-4 rounded-xl border border-dashed border-gray-200 bg-linear-to-b from-white to-gray-50/30">
                ...
                <p className="text-xs text-gray-650 leading-relaxed font-sans">
                  {lang === 'zh' ? (
                    <>
                      {trendDirection === 'upward' &&
                        '你的学术成长曲线呈现出非常健康的向上拉升趋势！这表明随着上课次数的增加，你的整体知识体系越来越巩固，学习耐力和答题准确率已经取得了决定性突破。'}
                      {trendDirection === 'stable' &&
                        '累积分数轨迹进入成熟的常态化稳定期。这反映出你表现相当均衡稳定。建议在保持住基础大盘的同时，试着探索更具挑战性的选拔赛或是高阶人工智能随堂测验来寻找新的学术突破。'}
                      {trendDirection === 'downward' &&
                        '近期累积平均分受某几次偶发失常作业的影响，增长势头有所受挫。请无需气馁，建议你到作业区查看教师留下的批注建议，利用微课复习针对差错进行重新作答。'}
                    </>
                  ) : (
                    <>
                      {trendDirection === 'upward' &&
                        'Your academic trajectory maps a highly competent upward lift. This strongly correlates with refined endurance, improved error resolution over consecutive modules, and a progressive understanding of core material.'}
                      {trendDirection === 'stable' &&
                        'Your running average sits in an equilibrium band. Your output exhibits solid consistency. Start aiming for advanced peer reviews or high-level AI topics to challenge your plateau and break into the higher bracket.'}
                      {trendDirection === 'downward' &&
                        'The moving average shows a slight descent due to standard deviations in recent assignments. Review the feedback timeline to recalibrate your core subjects and raise your general competency.'}
                    </>
                  )}
                </p>
              </div>
            ...
            <div className="mt-4 pt-3.5 border-t border-gray-100 space-y-2 select-none">
              ...
              <div className="flex items-center gap-2 text-xs font-bold text-gray-450">
                ...
                <span>
                  ...
                  <span className="text-slate-800 font-mono">{(stats.peakAvg - stats.valleyAvg).toFixed(1)}%</span>
                </span>
              </div>
            </div>
          </div>
