...
import { useErrorStore, formatBatchErrorReport, formatSingleErrorReport } from '../../store/errorStore';
...
export function SystemErrorCenterModal() {
  ...
  const handleCopyAll = async () => {
    const markdown = formatBatchErrorReport(errors);
  ...
  const handleCopyItem = async (e: React.MouseEvent, item: SystemErrorItem) => {
    e.stopPropagation();
    const markdown = formatSingleErrorReport(item);
    const success = await copyToClipboard(markdown);
    if (success) {
  ...
  return (
    <>
      {/* 1. Floating Diagnostics Pill Badge */}
      {errors.length > 0 && !isOpen && !isPillDismissed && (
        <motion.div
