              ...
              {suggestedQuestions.length === 0 && (
                <div className="space-y-4">
                  <div className="bg-indigo-50/80 p-4 rounded-xl border border-indigo-100 text-xs text-indigo-800 leading-relaxed font-sans">
                    Choose a lesson to scan. Our advanced AI model will run a deep semantic scan across the entire
                    lesson curriculum content, discover your core learning objectives, and construct interactive
                    multiple-choice questions aligning precisely with each of them.
                  </div>
                  ...
                  <div className="pt-4 flex justify-end font-sans">
                    <button
                      disabled={
                        isGeneratingSuggestions ||
                        (quizGenMode === 'scan_lesson' && !quizGenSelectedLessonId) ||
                        (quizGenMode === 'topic' && !quizGenTopic.trim())
                      }
                      onClick={async () => {
                ...
                <div className="space-y-6">
                  ...
                  <div className="font-sans">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {suggestedObjectives.map((obj, i) => (
                        <span
                          key={i}
                          className="px-2.5 py-1 bg-teal-50 text-teal-800 border border-teal-200 rounded-full text-xs font-semibold"
                        >
                          🎯 {obj}
                        </span>
                      ))}
                  ...
                  <div className="space-y-4 font-sans">
                    ...
                    <div className="space-y-3">
                      {suggestedQuestions.map((q, idx) => (
                        <div
                          key={idx}
                          className="p-4 rounded-xl border border-gray-150 bg-gray-50/50 hover:bg-gray-50 transition space-y-3 text-gray-800 text-gray-850"
                        >
                          ...
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            {q.options.map((opt: string, optIdx: number) => {
                              ...
                              return (
                                <div
                                  key={optIdx}
                                  className={`p-2 rounded border flex items-center justify-between ${isCorrect ? 'bg-green-50 border-green-200 text-green-955 font-semibold' : 'bg-white border-gray-100 text-gray-700'}`}
                                >
                                  <span>{opt}</span>
                  ...
                  <div className="pt-4 border-t border-gray-100 flex justify-between items-center font-sans">
                    ...
                    <div className="flex items-center gap-2 border border-indigo-150 bg-indigo-50/50 px-3 py-1.5 rounded-lg text-xs font-semibold text-indigo-900">
                      ...
                      >
                        <option className="text-gray-800" value={0}>
                          No Limit
                        </option>
                        <option className="text-gray-800" value={1}>
                          1 Min
                        </option>
                        <option className="text-gray-800" value={2}>
                          2 Mins
                        </option>
                        <option className="text-gray-800" value={5}>
                          5 Mins
                        </option>
                        <option className="text-gray-800" value={10}>
                          10 Mins
                        </option>
                        <option className="text-gray-800" value={15}>
                          15 Mins
                        </option>
                        <option className="text-gray-800" value={20}>
                          20 Mins
                        </option>
                        <option className="text-gray-800" value={30}>
                          30 Mins
                        </option>
                        <option className="text-gray-800" value={45}>
                          45 Mins
                        </option>
                        <option className="text-gray-800" value={60}>
                          60 Mins
                        </option>
                      </select>
                    </div>
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          const res = await fetch(
                            `/api/classes/${quizGeneratorClassId}/assignments/create-suggested-quiz`,
                            {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
                              body: JSON.stringify({
