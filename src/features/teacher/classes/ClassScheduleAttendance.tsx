    ...
    <div className="mb-4 bg-white p-3 rounded-xl border border-slate-100 shadow-xs">
      ...
      <div className="mb-3 flex gap-2 items-center">
        <input
          title="Schedule Date"
          type="date"
          className="border border-slate-200 hover:border-slate-300 rounded-lg text-xs p-1.5 flex-1 bg-white focus:outline-hidden focus:ring-1 focus:ring-indigo-500 text-gray-750 transition-all font-sans"
        ...
        <select
          ...
          className="border border-slate-200 hover:border-slate-300 rounded-lg text-xs p-1.5 flex-1 bg-white focus:outline-hidden focus:ring-1 focus:ring-indigo-500 text-gray-750 transition-all font-sans cursor-pointer"
          value={newScheduleLessonId}
          onChange={(e) => setNewScheduleLessonId(e.target.value)}
          onClick={(e) => e.stopPropagation()}
        >
        ...
        {(classSchedulesMap[cls.id] || []).length === 0 ? (
          ...
          classSchedulesMap[cls.id].map((sch) => {
            ...
            return (
              <div
                key={sch.id}
                className="bg-white border border-slate-100/90 rounded-xl shadow-xs hover:shadow-sm transition-all overflow-hidden mb-2"
              >
                <div
                    ...
                    <div className="grid gap-1">
                      {cStudents.map((st) => {
                        ...
                        return (
                          <div
                            key={st.id}
                            className="flex items-center justify-between text-xs bg-white p-1 rounded border border-gray-200"
                          >
                            <div className="flex items-center gap-1.5 truncate max-w-[150px]">
                              <span className="font-medium text-gray-700 truncate" title={st.name}>
                                {st.name}
                              </span>
                              {(() => {
