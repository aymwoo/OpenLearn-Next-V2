...
export interface ClassStudentsPanelProps {
  ...
  lessons: Lesson[];
  setStudents:
    | React.Dispatch<React.SetStateAction<StudentType[]>>
...
export function ClassStudentsPanel({
  ...
  return (
    <div className="mb-4 bg-white p-3 rounded-xl border border-slate-100 shadow-xs">
        ...
        <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
            ...
            <>
              ...
              >
                <option value="" disabled>
                  {lang === 'zh' ? '添加已有学生...' : 'Enroll existing...'}
                </option>
                {students
                  .filter((st) => !cStudents.some((cs) => cs.id === st.id))
                  .map((st) => (
                    <option key={st.id} value={st.id}>
                      {st.name} {st.student_number ? `(${st.student_number})` : `(${st.id.slice(0, 6)})`}
                    </option>
                  ))}
              </select>
              <button
          ...
          <button
            onClick={async (e) => {
              ...
              if (!name) return;
              const studentNumberPrompt = window.prompt(
                lang === 'zh'
                  ? '请输入学号 (可选，留空将自动生成):'
                  : 'Enter student number (optional, leave blank to auto-generate):',
              );
              ...
              const createRes = await fetch('/api/students', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name, email, password, ...(student_number ? { student_number } : {}) }),
              });
              if (createRes.ok) {
            ...
            onChange={async (e) => {
              ...
              reader.onload = async (evt) => {
                ...
                if (studentsToEnroll.length === 0) {
                  alert(
                    lang === 'zh'
                      ? '未能识别出有效的学生数据。请确保 CSV 文件包含 "学生姓名" (或 "name")，可选包含 "学号" (student_number) 和 "学生邮箱" (email) 字段。'
                      : 'No valid student records found. Maintain at least a "name" column (and optional "student_number") in your CSV.',
                  );
                  return;
                }
      ...
      {cStudents.length === 0 ? (
        <div className="text-xs text-gray-500 italic p-1 text-left">
          {lang === 'zh' ? '该班级暂无学生。可以使用右侧按钮添加或注册学生' : 'No students registered in this class.'}
        </div>
      ) : (
        (() => {
          const filtered = cStudents.filter((st) => {
            ...
            return (
              ...
              (st.student_number && st.student_number.toLowerCase().includes(q)) ||
                  ...
                  >
                    {filtered.map((st) => {
                      ...
                      return (
                        <div
                          key={st.id}
                          className="border border-slate-100/75 w-full min-w-0 flex flex-col bg-white rounded-xl p-2.5 shadow-xs hover:border-slate-200 hover:shadow-sm transition-all duration-200 text-left"
                        >
                          <div
                                        ...
                                        <span
                                          ...
                                          title={
                                            lang === 'zh'
                                              ...
                                              : `30-day average has dropped below 60% (${avg30}%)`
                                          }
                                        >
                              ...
                              <button
                                ...
                                onClick={async (e) => {
                                  if (
                                    window.confirm(
                                      lang === 'zh'
                                        ? `确定要将学生 [${st.name}] 从本班级移除吗？`
                                        : `Remove student [${st.name}] from this class?`,
                                    )
                                  ) {
                          ...
                          {isStExpanded && (
                            <div className="pl-6 pb-2 pr-2 text-left">
                              {/* Student level Tabs */}
                              <div
                                className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl mb-3 max-w-[320px] border border-slate-200/40"
                                onClick={(e) => e.stopPropagation()}
                              >
                                <button
                                  onClick={(e) => {
                              ...
                              {stActiveTab === 'settings' ? (
                                <div className="space-y-3" onClick={(e) => e.stopPropagation()}>
                                  <div className="mt-1 flex items-center justify-between text-[11px] p-1.5 bg-gray-150/35 rounded border border-gray-100">
                                    ...
                                    >
                                      <option value="">
                                        {lang === 'zh' ? '无 (自主学习)' : 'None (Free Dashboard)'}
                                      </option>
                                      {lessons.map((l) => (
                                      ...
                                      <input
                                        ...
                                        onChange={async (e) => {
                                          ...
                                          await fetch(`/api/students/${st.id}`, {
                                            method: 'PUT',
                                            headers: { 'Content-Type': 'application/json' },
                                            body: JSON.stringify({ student_number: newNumber }),
                                          });
                                          setStudents((prev) =>
                                            prev.map((s) => (s.id === st.id ? { ...s, student_number: newNumber } : s)),
                                          );
                                          if (classStudentsMap[cls.id]) {
                                            setClassStudentsMap((prev) => {
                                              ...
                                              return {
                                                ...prev,
                                                [cls.id]: list.map((s) =>
                                                  s.id === st.id ? { ...s, student_number: newNumber } : s,
                                                ),
                                              };
                                            });
                                          }
                                        ...
                                        onClick={(e) => e.stopPropagation()}
                                        title={lang === 'zh' ? '点击可修改学号' : 'Click to edit student number'}
                                      />
                                      {st.student_number && (
                                        <button
                                          type="button"
                                          onClick={(e) =>
                                            handleCopyStudentNumber(e, st.student_number!, `settings-${st.id}`)
                                          }
                                          title={lang === 'zh' ? '复制学号' : 'Copy student number'}
                                          className="p-1 text-gray-400 hover:text-indigo-600 transition-colors cursor-pointer"
                                        >
                                          {copiedStudentId === `settings-${st.id}` ? (
                                  ...
                                  <div className="flex items-center justify-between text-[11px] p-1.5 bg-gray-150/35 rounded border border-gray-100 mt-1">
                                    ...
                                    <input
                                      ...
                                      onChange={async (e) => {
                                        ...
                                        setStudents((prev) =>
                                          prev.map((s) => (s.id === st.id ? { ...s, password: newPwd } : s)),
                                        );
                                      }}
                                      onClick={(e) => e.stopPropagation()}
                                  ...
                                  <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                    ...
                                    >
                                      ...
                                      {lessons
                                        .filter((l) => !progress.some((p) => p.lesson_id === l.id))
                                        .map((l) => (
                                          <option key={l.id} value={l.id}>
                                            {l.title}
                                          </option>
                                ...
                                <div className="mt-1" onClick={(e) => e.stopPropagation()}>
                                  <StudentPrivateNotesEditor
                                    ...
                                    onSave={async (newNotes) => {
                                      ...
                                      if (res.ok) {
                                        setStudents((prev) =>
                                          prev.map((s) => (s.id === st.id ? { ...s, private_notes: newNotes } : s)),
                                        );
                                        if (classStudentsMap[cls.id]) {
                                          setClassStudentsMap((prev) => {
                                            const list = prev[cls.id] || [];
                                            return {
                                              ...prev,
                                              [cls.id]: list.map((s) =>
                                                s.id === st.id ? { ...s, private_notes: newNotes } : s,
                                              ),
                                            };
                                          });
                                        }
                                ...
                                <div className="space-y-2" onClick={(e) => e.stopPropagation()}>
                                  {progress.length === 0 ? (
                                    <div className="text-xs text-gray-500 italic">
                                      {lang === 'zh' ? '未分配任何课程。' : 'No assigned lessons.'}
                                    </div>
                                  ) : (
                                    <div className="flex flex-col gap-2">
                                      {progress.map((p) => (
                                        <div
                                          key={p.lesson_id}
                                          className="text-xs flex items-center justify-between pr-2"
                                        >
                                          <span
                                            className="truncate max-w-[130px] font-semibold text-gray-750"
                                          ...
                                          <div className="flex-1 mx-2 h-1.5 bg-gray-200 rounded-full overflow-hidden shrink-0">
                                            <div
                                              className={`h-full ${p.progress_percent === 100 ? 'bg-green-500' : 'bg-indigo-500'}`}
