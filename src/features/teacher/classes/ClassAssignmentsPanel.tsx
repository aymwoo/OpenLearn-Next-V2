              ...
              {classDashboardMap[cls.id].assignments &&
                (() => {
                  ...
                  const processed = rawAssignments.map((ast: any) => {
                    ...
                    const gradedScores = astPerf
                      .filter((p: any) => p.score !== null && p.score !== undefined)
                      .map((p: any) => Number(p.score));
                    const avgScore =
                      gradedScores.length > 0
                        ? Math.round(gradedScores.reduce((a: number, b: number) => a + b, 0) / gradedScores.length)
                        : null;
                  ...
                  return sorted.map((ast: any) => (
                    ...
                    <div
                      key={ast.id}
                      className="bg-white p-3 rounded-xl border border-gray-150 hover:border-indigo-200 shadow-sm text-xs cursor-pointer hover:shadow transition-all flex flex-col justify-between"
                    >
                      <div className="flex items-start justify-between gap-1.5 mb-1.5">
                        <div className="font-semibold text-gray-800 line-clamp-1 flex-1 font-sans" title={ast.title}>
                          {ast.title}
                        </div>
                        <span
                      ...
                      <div className="flex items-center justify-between border-t border-gray-55 pt-2 mt-auto gap-2">
                        ...
                        <button
                          onClick={async (e) => {
                            ...
                            if (subText) {
                              const res = await fetch(`/api/assignments/${ast.id}/submissions`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ studentId: s.id, content: subText }),
                              });
              ...
              <div className="flex items-center gap-1 bg-gray-50 p-0.5 rounded-lg border border-gray-200">
                {(['all', 'submitted', 'graded', 'pending'] as const).map((filterOpt) => {
                  ...
                  const optCounts = (() => {
                    ...
                    if (filterOpt === 'submitted')
                      return recentSubs.filter((s: any) => s.status === 'submitted').length;
                    ...
                    if (filterOpt === 'pending') {
                      ...
                      const pStudent = performanceData.filter(
                        (p: any) => !p.submission_status || p.submission_status === null,
                      ).length;
                ...
                <div
                  key={
                    sub.status === 'pending_student'
                      ? `${sub.assignment_id}-${sub.student_id}-pending-${idx}`
                      : `${sub.assignment_id}-${sub.student_id}-${idx}`
                  }
                  className="bg-white p-2 border border-gray-150 rounded-lg text-xs flex justify-between items-center group hover:border-gray-300 transition-colors shadow-none mt-0.5"
                >
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const res = await fetch(
                              `/api/assignments/${sub.assignment_id}/submissions/${sub.student_id}/grade`,
                              { method: 'POST' },
                            );
                            if (res.ok) await fetchClassDashboard(cls.id);
                          } finally {
              ...
              <div className="overflow-x-auto border border-gray-200 rounded">
                <table className="w-full text-xs text-left bg-white">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      <th className="p-2 border-b border-r border-gray-200 font-medium whitespace-nowrap text-gray-600">
                        Student
                      </th>
                      {classDashboardMap[cls.id].assignments.map((a: any) => (
                        <th
                          key={a.id}
                          className="p-2 border-b border-r border-gray-200 font-medium truncate max-w-[80px]"
                          title={a.title}
                        >
                          {a.title}
                        </th>
                  ...
                  <tbody>
                    {cStudents.map((st) => (
                      <tr key={st.id} className="border-b border-gray-100 last:border-b-0">
                        ...
                        {classDashboardMap[cls.id].assignments.map((a: any) => {
                          const perf = classDashboardMap[cls.id].performance.find(
                            (p: any) => p.assignment_id === a.id && p.student_id === st.id,
                          );
                          ...
                          return (
                            <td
                              key={a.id}
                              className={`p-2 border-r border-gray-100 text-center relative group/cell ${bgClass}`}
                            >
                              {(() => {
                                      ...
                                      <div className="absolute bottom-full mb-1.5 left-1/2 -translate-x-1/2 hidden group-hover/cell:block bg-gray-900 border border-gray-800 text-white text-xs p-2.5 rounded-xl shadow-2xl z-30 w-44 pointer-events-none text-left leading-normal font-sans font-normal normal-case">
                                        ...
                                        {formattedGradedTime && (
                                          <div className="text-gray-300 flex items-center gap-1 font-semibold text-xs mb-1">
                                            ...
                                            <span>
                                              {lang === 'zh'
                                                ? `时间: ${formattedGradedTime}`
                                                : `Graded: ${formattedGradedTime}`}
                                            </span>
                                          </div>
                                        )}
                                        {perf.feedback && (
