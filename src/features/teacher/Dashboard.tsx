                ...
                {nextClass ? (
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <h3
                        className="text-lg md:text-xl font-extrabold tracking-tight mt-3 text-slate-800 line-clamp-2"
                        title={nextClass.lesson_title}
                      >
                        {nextClass.lesson_title}
                      </h3>
                      <p className="text-xs text-slate-500 mt-2 font-medium">
                        📍 {nextClass.class_name} {nextClass.classroom && ` | 教室: ${nextClass.classroom}`}{' '}
                        {nextClass.teacher_name && ` | 教师: ${nextClass.teacher_name}`}
                      </p>
                    </div>
                    {nextClass.notes && (
                      <div
                        className="mt-3 text-xs italic text-indigo-700 bg-indigo-50/60 p-2 rounded-lg border border-indigo-100/40 truncate"
                        title={nextClass.notes}
                      >
                        * {nextClass.notes}
                      </div>
                    )}
                  ...
                  <div className="flex-1 flex flex-col justify-center items-center text-center py-6 select-none">
                    ...
                    <p className="text-xs text-slate-455 mt-1 max-w-[280px]">
                      {lang === 'zh'
                        ? '接下来暂无排定课次，您可以休息调整。'
                        : 'No remaining schedules today. Take a rest!'}
                    </p>
                  </div>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          <div className="bg-white border border-rose-100 rounded-2xl shadow-sm flex flex-col transition-all duration-300">
            ...
            {!isApprovalsCollapsed && (
              <div className="flex-1 overflow-y-auto p-3 max-h-[350px]">
                {approvals.length === 0 ? (
                  <div className="text-center p-8 text-xs md:text-sm text-slate-400 font-medium">{t.noApprovals}</div>
                ) : (
                  approvals.map((approval) => {
                    ...
                    const currentScore =
                      scoreOverrides[approval.id] !== undefined ? scoreOverrides[approval.id] : payload.score || 0;

                    return (
                      <div
                        key={approval.id}
                        className="w-full p-4 rounded-xl text-sm bg-slate-55 border border-slate-100 mb-3 shadow-3xs flex flex-col gap-3 hover:border-rose-150 transition-colors"
                      >
                        <div>
                          <div className="font-extrabold text-slate-800 mb-1">
                            {approval.command_type === 'ai.apply_grade'
                              ? '🎓 Evaluate Submission'
                              : approval.command_type}
                          </div>
                          <div
                            className="text-xs text-slate-500 font-mono mb-2 line-clamp-2 bg-white border border-slate-100 p-2 rounded-lg"
                        ...
                        {isGrade && (
                          <div className="bg-white border border-slate-150 p-2 rounded-xl flex items-center justify-between gap-3">
                            ...
                            <input
                              ...
                              value={currentScore}
                              onChange={(e) =>
                                setScoreOverrides((prev) => ({ ...prev, [approval.id]: parseInt(e.target.value) || 0 }))
                              }
                            />
                          </div>
                        )}
                        <div className="flex gap-2 justify-end mt-1">
                          <button
                            onClick={() =>
                              handleApprove(
                                approval.id,
                                isGrade && scoreOverrides[approval.id] !== undefined
                                  ? { score: scoreOverrides[approval.id] }
                                  : undefined,
                              )
                            }
          ...
          <div className="bg-white border border-slate-200/60 rounded-2xl shadow-sm flex flex-col transition-all duration-300">
            ...
            {!isProcessesCollapsed && (
              <div className="flex-1 overflow-y-auto p-3 max-h-[350px]">
                {processes.length === 0 ? (
                  <div className="text-center p-8 text-xs md:text-sm text-slate-400 font-medium">{t.noProcesses}</div>
                ) : (
                  processes.map((proc) => (
                    <div
                      key={proc.id}
                      className="w-full p-4 rounded-xl text-sm bg-slate-50 border border-slate-100 mb-3 shadow-3xs flex flex-col gap-2.5 hover:border-indigo-150 transition-colors"
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-extrabold text-slate-800 truncate" title={proc.name}>
                          {proc.name}
                        </span>
                        <span
