...
export const CommandBusPlayground: React.FC<CommandBusPlaygroundProps> = ({ registeredCommands, onRefresh }) => {
  const [search, setSearch] = useState('');
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'vfs' | 'edu' | 'mgmt' | 'proc' | 'ai' | 'plugin'>(
    'all',
  );
  const [expandedCommandId, setExpandedCommandId] = useState<string | null>(null);
  const [commandPayloads, setCommandPayloads] = useState<Record<string, string>>({});
  const [executionResults, setExecutionResults] = useState<
    Record<string, { success: boolean; data?: any; error?: string; loading?: boolean }>
  >({});
  ...
  const handleExecute = async (commandType: string, cmdId: string) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setExecutionResults((prev) => ({
          ...prev,
          [cmdId]: { success: false, error: data.error || '执行命令失败', loading: false },
        }));
      }
    } catch (err: any) {
                            ...
                            <div className="space-y-3 font-mono text-[11px]">
                              {Object.keys(cmd.inputSchema.properties).map((propName) => {
                                ...
                                return (
                                  <div
                                    key={propName}
                                    className="flex flex-col gap-1 border-b border-gray-50 last:border-b-0 pb-1.5"
                                  >
                                    ...
                                    {prop.description && (
                                      <span className="text-gray-500 font-sans leading-relaxed text-xs">
                                        {prop.description}
                                      </span>
                                    )}
                                  </div>
                          ...
                          <div className="font-bold flex items-center gap-1.5 mb-1 bg-transparent border-0 p-0 text-xs text-neutral-800">
                            {execResult.loading ? (
                              ...
                              <span className="text-green-700 flex items-center gap-1">
                                <CheckCircle2 size={14} /> STATUS: 200 SUCCESS (Action Completed)
                              </span>
                            ) : (
                              <span className="text-red-700 flex items-center gap-1">
                                <X size={14} /> STATUS: 500 INTERNAL_BUS_ERROR
                              </span>
                            )}
                          </div>
