...
export function StudentAssignmentsPanel(props: StudentAssignmentsPanelProps) {
  const { assignments, setSelectedAssignment, setStudentViewStatus, setQuizStudentAnswers, setSubAssignmentTab, lang } =
    props;
  return (
        ...
        {assignments.length === 0 ? (
          ...
          <div className="space-y-3">
            {assignments.map((ast: any) => (
              <div
                key={ast.id}
                className="flex flex-col p-3 rounded-lg border border-gray-100 bg-gray-50 hover:bg-gray-100 transition-colors"
              >
                ...
                <div className="text-xs text-gray-500 mb-2">
                  {ast.class_name} &middot; <span className="text-gray-400 italic">{ast.content}</span>
                </div>
                ...
                {ast.submission_status === 'graded' && ast.feedback && (
                  <div className="mt-2 bg-green-50 p-2.5 rounded-lg border border-green-100 flex flex-col gap-1 text-xs text-green-800">
                    ...
                    <span className="leading-snug bg-green-50/20 rounded p-1 text-xs text-gray-700 whitespace-pre-wrap">
                      {ast.feedback}
                    </span>
                  </div>
                )}
