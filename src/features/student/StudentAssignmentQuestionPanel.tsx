    ...
    <div className="w-1/3 border-r border-gray-100 pr-4 overflow-y-auto hidden md:block">
      ...
      {selectedAssignment?.content && selectedAssignment.content.startsWith('{"quizType":"mcq_learning_objectives"') ? (
        <div className="space-y-4">
          ...
          <p className="text-xs text-gray-500 leading-relaxed font-sans">
            This assessment was automatically mapped to the core learning objectives of your lesson by our tutoring
            assistant compiler.
          </p>
          {(() => {
            try {
      ...
      {!selectedAssignment.submission_status && (
        <div className="mt-8 border-t border-gray-100 pt-6">
          ...
          <p className="text-xs text-gray-500 mb-4">
            ...
            selectedAssignment.content.startsWith('{"quizType":"mcq_learning_objectives"')
              ...
              if (isMcq) {
                try {
                  ...
                  if (answeredCount < parsed.questions.length) {
                    if (
                      !window.confirm(
                        `You have only answered ${answeredCount}/${parsed.questions.length} questions. Are you sure you want to submit your answers?`,
                      )
                    ) {
                      return;
                    }
      ...
      {selectedAssignment.submission_status === 'graded' && selectedAssignment.feedback && (
        <div className="mt-6 bg-green-50 p-4 rounded-xl border border-green-100">
          <div className="font-semibold text-green-800 text-sm mb-1 flex items-center gap-1">
            <CheckCircle2 size={16} /> Grade: {selectedAssignment.score}%
          </div>
          <div className="text-xs text-green-700 whitespace-pre-wrap leading-relaxed font-sans">
