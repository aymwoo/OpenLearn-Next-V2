        ...
        return (
          <>
            ...
            {isMcqQuiz && subAssignmentTab === 'quiz' ? (
                      ...
                      <>
                        ...
                        {parsed.questions.map((q: any, idx: number) => {
                          ...
                          return (
                            <div
                              key={idx}
                              className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm space-y-3 font-sans"
                            >
                              ...
                              <div className="font-semibold text-gray-800 text-sm">{q.question}</div>
                              ...
                              <div className="grid grid-cols-2 gap-3 text-xs pt-1">
                                {q.options.map((opt: string, optIdx: number) => {
                                  ...
                                  let optStyle =
                                    'border-gray-200 hover:border-gray-300 bg-white text-gray-700 hover:bg-gray-50/50 cursor-pointer';
                                  ...
                                  if (isSubmitted) {
                                    ...
                                    optStyle =
                                      'border-indigo-600 bg-indigo-50/30 text-indigo-900 ring-2 ring-indigo-100 font-medium cursor-pointer';
                                  }
              ...
              <div
                className={`flex-1 min-h-0 flex flex-col ${selectedAssignment.submission_status ? 'opacity-90 pointer-events-none filter grayscale-[0.2]' : ''}`}
              >
                <LazyWhiteboard
                  ...
                  onElementAdd={async (type: string, data: any) => {
                    await fetch(
                      `/api/lessons/assignment-${selectedAssignment.id}-student-${activeStudentId}/whiteboard`,
                      {
                        method: 'POST',
                  ...
                  onElementUpdate={async (elementId: string, data: any) => {
                    await fetch(
                      `/api/lessons/assignment-${selectedAssignment.id}-student-${activeStudentId}/whiteboard/${elementId}`,
                      {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
