...
export function NavigationSidebar({
  ...
  return (
    <div
      ...
      className={`${mainNavCollapsed ? 'w-16' : 'w-16 md:w-64'} bg-surface border-r border-theme text-main flex flex-col transition-all duration-300`}
    >
      ...
      <div className={`p-2 ${mainNavCollapsed ? 'md:p-2' : 'md:p-4'} flex flex-col gap-3 overflow-y-auto flex-1`}>
        ...
        {session?.subRole === 'administrator' && (
          <button
            ...
            className={`flex items-center gap-3 p-3 transition-colors text-sm font-medium text-indigo-700 hover:bg-indigo-50 border border-slate-200/50 rounded-xl ${teacherTab === 'admin_directory' ? 'bg-indigo-50/70 border-indigo-200' : 'bg-slate-50/50'} ${mainNavCollapsed ? 'justify-center px-2' : ''}`}
            title={lang === 'zh' ? '管理后台' : '⭐ Admin Center'}
          >
            <Shield size={20} className="shrink-0 text-indigo-600 animate-pulse" />
            <span className={mainNavCollapsed ? 'hidden' : 'hidden md:block font-bold text-indigo-850'}>
      ...
      {(() => {
        ...
        return (
          <div
            className="mt-auto p-3 m-3 bg-indigo-50/55 rounded-xl border border-indigo-105 hidden md:block select-none shadow-3xs"
