...
async function vfsWrite(ctx: PluginContext, path: string, content: string): Promise<boolean> {
  try {
    await ctx.services.commandBus.execute({
      commandType: 'vfs.write_file',
      payload: { path, content },
      actorId: 'plugin:courseware-hub',
    });
    return true;
  } catch (e: any) {
    ctx.log.warn(`VFS write failed for ${path}: ${e.message}`);
    return false;
  }
}

async function vfsRead(ctx: PluginContext, path: string): Promise<string | null> {
  try {
    const result = await ctx.services.commandBus.execute({
      commandType: 'vfs.read_file',
      payload: { path },
      actorId: 'plugin:courseware-hub',
    });
    return (result as any)?.content || null;
  } catch (e: any) {
    ctx.log.warn(`VFS read failed for ${path}: ${e.message}`);
    return null;
  }
}

async function vfsDelete(ctx: PluginContext, path: string): Promise<void> {
  try {
    await ctx.services.commandBus.execute({
      commandType: 'vfs.delete_file',
      payload: { path },
      actorId: 'plugin:courseware-hub',
    });
  } catch (e: any) {
  ...
  async activate(ctx: PluginContext) {
    ...
    await commandBus.registerHandler('courseware.upload', {
      async execute(cmd) {
        ...
        if (!hasSDKCall(htmlContent)) {
          try {
            const aiResponse = await ctx.services.ai.generate({
              prompt: buildAnalysisPrompt(htmlContent),
              maxTokens: 1024,
            });
        ...
        const origPath = `${VFS_BASE}/${coursewareKey}/v${nextVersion}/original.html`;
        const injectedPath = `${VFS_BASE}/${coursewareKey}/v${nextVersion}/injected.html`;

        const ok1 = await vfsWrite(ctx, origPath, htmlContent);
        const ok2 = await vfsWrite(ctx, injectedPath, injected);
        ...
        `).run(
          ...
          origPath, injectedPath, JSON.stringify(extractionConfig),
    ...
    await commandBus.registerHandler('courseware.delete', {
      async execute(cmd) {
        ...
        for (const v of allVersions) {
          await vfsDelete(ctx, v.html_vfs_path);
          await vfsDelete(ctx, v.injected_html_vfs_path);
        }
    ...
    await commandBus.registerHandler('courseware.get_html', {
      async execute(cmd) {
        ...
        const html = await vfsRead(ctx, row.injected_html_vfs_path);
    ...
    await commandBus.registerHandler('courseware.update_extraction', {
      async execute(cmd) {
        ...
        // 重新生成注入版本并写回 VFS
        const origHtml = await vfsRead(ctx, row.html_vfs_path);
        ...
        await vfsWrite(ctx, row.injected_html_vfs_path, injected);
    ...
    await commandBus.registerHandler('courseware.get_original_html', {
      async execute(cmd) {
        ...
        if (!row) return { error: 'not_found' };
        const html = await vfsRead(ctx, row.html_vfs_path);
        if (!html) return { error: 'vfs_read_failed' };
        return { html };
      },
    });
