...
### Next-round backlog
- **Frontend monolith (`src/App.tsx`)** is the active decomposition target for `0.3.0` — extracted incrementally by feature area with characterization tests. The `classes` module is now fully decomposed (Phase 2 done). The largest remaining inline region is the **student-view** (`studentViewStatus === 'lesson' | 'assignment' | 'dashboard'`) branch and other teacher-tab bodies; each should be decomposed the same way (verbatim move + characterization test, baseline held at 116 `tsc` errors).
