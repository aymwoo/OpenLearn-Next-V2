...
### Next-round backlog
- **Frontend monolith (`src/App.tsx`)** is the active decomposition target for `0.3.0` — extracted incrementally by feature area with characterization tests. Phases 1–3 are done: `lesson_editor`, `classes`, and the `student` view are now fully decomposed behind `LessonEditorView`, `ClassesView`, and `StudentView`. The largest remaining inline regions are the other **teacher-tab bodies** (`courses`, `whiteboard`, `homework`, `live_class`, `management`, etc.) still rendered inline in `src/App.tsx`; each should be decomposed the same way (verbatim move + characterization test, baseline held at 116 `tsc` errors).
