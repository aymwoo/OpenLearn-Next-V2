...
### Next-round backlog
- **Frontend monolith (`src/App.tsx`)** is the active decomposition target for `0.3.0` — extracted incrementally by feature area with characterization tests. Phases 1–4 are done: `lesson_editor`, `classes`, `student`, and the entire `teacher` branch are now decomposed behind `LessonEditorView`, `ClassesView`, `StudentView`, and `TeacherView`. The remaining inline regions in `src/App.tsx` are the top header / role-switch shell, the `RightSidebar`, and the various modals / dialogs still rendered inline; these can be decomposed the same way (verbatim move + characterization test, baseline held at 116 `tsc` errors).
