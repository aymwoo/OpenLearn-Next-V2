...
// Socket service is injected via MfeContext DI (D-05)
// import { io, Socket } from 'socket.io-client'; // REMOVED: use DI instead

/** MfeContext type for DI integration */
...
/** ISocketService interface for type-safe DI usage */
...
interface InteractiveWhiteboardProps {
  ...
  userRole?: 'teacher' | 'student';
  isEditMode?: boolean;
  /** MfeContext injected by host via createMfeApp factory */
...
export function InteractiveWhiteboard({ 
  ...
  // Use DI-injected ISocketService from MfeContext instead of direct io() (D-05)
  useEffect(() => {
    const socketService = mfeContext?.serviceRegistry?.get<ISocketService>('@openlearn/frontend:ISocketService');
    if (!socketService) {
      // Fallback: If no DI context available, socket features are disabled
      console.warn('[InteractiveWhiteboard] No ISocketService available via MfeContext DI');
      return;
    }
    ...
    return () => {
      ...
      // Note: Do NOT call disconnect() — the socket is a host singleton managed by MfeContext
    }
  }, [lessonId, onRefresh, onSegmentSync, mfeContext]);
              ...
              {!data.isMinimized && (
                <div className="flex-1 bg-white overflow-hidden relative min-h-0">
                  <iframe
                    ...
                    srcDoc={data.coursewareUuid || data.resourceId ? undefined : data.code}
