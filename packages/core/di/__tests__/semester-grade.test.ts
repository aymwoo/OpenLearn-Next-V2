...
describe('SemesterGradeService - Database Schemas & Sync Logic (Wave 2)', () => {
  ...
  beforeAll(() => {
    ...
    db.prepare(
      'INSERT OR REPLACE INTO lessons (id, title, content, created_at, updated_at) VALUES (?, ?, ?, ?, ?)',
  ...
  it('should verify that plugin_submissions, plugin_peer_reviews, and plugin_grades tables exist', () => {
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'plugin_%'").all() as {
      name: string;
    }[];
...
describe('SemesterGradeService & AssignmentEvalPlugin Integration (Wave 3)', () => {
  ...
  it('should handle assignment.submit successfully and increment version on duplicate submit', async () => {
    ...
    const cmd1 = kernel.commandBus.createCommand(
      'assignment.submit',
      {
    ...
    const cmd2 = kernel.commandBus.createCommand(
      'assignment.submit',
      {
        lessonId: testLessonId,
        studentId: testStudentId1,
  ...
  it('should block self peer reviews and allow peer review from other students', async () => {
    // Alice submits
    const cmdSubmitAlice = kernel.commandBus.createCommand(
      'assignment.submit',
      {
        lessonId: testLessonId,
        studentId: testStudentId1,
    ...
    // Bob submits
    const cmdSubmitBob = kernel.commandBus.createCommand(
      'assignment.submit',
      {
        lessonId: testLessonId,
    ...
    const cmdSelfReview = kernel.commandBus.createCommand(
    ...
    const cmdInvalidReview1 = kernel.commandBus.createCommand(
      'assignment.peer_review',
      {
        submissionId: aliceSubId,
    ...
    const cmdInvalidReview2 = kernel.commandBus.createCommand(
      'assignment.peer_review',
      {
        submissionId: aliceSubId,
        reviewerId: testStudentId2,
    ...
    const cmdValidReview = kernel.commandBus.createCommand(
      'assignment.peer_review',
      {
        submissionId: aliceSubId,
        reviewerId: testStudentId2,
  ...
  it('should handle grading as draft or confirmed with correct weights and score calculation', async () => {
    ...
    const cmdReview1 = kernel.commandBus.createCommand(
      'assignment.peer_review',
      {
    ...
    // 1. Grade as Draft
    const cmdGradeDraft = kernel.commandBus.createCommand(
      'assignment.grade',
      {
        submissionId: bobSubId,
        teacherScore: 90,
    ...
    // 3. Defensive checks
    // Invalid score
    const cmdInvalidGrade = kernel.commandBus.createCommand(
      'assignment.grade',
      {
        submissionId: bobSubId,
        teacherScore: -10,
    ...
    const cmdInvalidWeights = kernel.commandBus.createCommand(
      'assignment.grade',
      {
        submissionId: bobSubId,
        teacherScore: 90,
