...
describe('Raffle & Vote Plugin Integration Test', () => {
  ...
  it('should create a vote, cast votes, and export results successfully', async () => {
    ...
    const RaffleVotePlugin = {
      ...
      activate: async (ctx: any) => {
        ...
        await ctx.db.ensureTable(
          'votes',
          'id TEXT PRIMARY KEY, lesson_id TEXT, title TEXT, options TEXT, element_ids TEXT, created_at INTEGER',
        );
          ...
          execute: async (command: any) => {
            ...
            const bgDraw = (await commandBus.execute({
              ...
              payload: {
                lessonId,
                type: 'rectangle',
                data: JSON.stringify({ x, y, width: 380, height: bgHeight, fill: '#f8fafc' }),
              },
            })) as any;
            ...
            for (let i = 0; i < options.length; i++) {
              const barDraw = (await commandBus.execute({
                ...
                payload: {
                  lessonId,
                  type: 'rectangle',
                  data: JSON.stringify({ x: x + 20, y: y + 50 + i * 45, width: 15, height: 18, fill: '#38bdf8' }),
                },
              })) as any;
            ...
            await dbInstance
              .prepare(
                'INSERT INTO ' +
                  tblVotes +
                  ' (id, lesson_id, title, options, element_ids, created_at) VALUES (?, ?, ?, ?, ?, ?)',
              )
              .run(
                voteId,
                lessonId,
                ...
                JSON.stringify({ bgId: bgDraw?.elementId, options: optionElementIds, x, y }),
                Date.now(),
              );
        ...
        await commandBus.registerHandler('vote.cast', {
          execute: async (command: any) => {
            ...
            const voteConfig = (await dbInstance
              .prepare('SELECT * FROM ' + tblVotes + ' WHERE id = ? AND lesson_id = ?')
              .get(voteId, lessonId)) as any;
            ...
            const checkVoted = (await dbInstance
              .prepare('SELECT count(*) as count FROM ' + tblCast + ' WHERE vote_id = ? AND voter_id = ?')
              .get(voteId, voterId)) as any;
            if (checkVoted.count > 0) throw new Error('不可重复投票');

            await dbInstance
              .prepare('INSERT INTO ' + tblCast + ' (vote_id, option_index, voter_id, timestamp) VALUES (?, ?, ?, ?)')
              .run(voteId, optionIndex, voterId, Date.now());
            const allVotes = (await dbInstance
              .prepare(
                'SELECT option_index, count(*) as count FROM ' + tblCast + ' WHERE vote_id = ? GROUP BY option_index',
              )
              .all(voteId)) as any[];
    ...
    db.prepare(
      'INSERT OR REPLACE INTO plugins (id, name, manifest, source_code, status, created_at, execution_mode) VALUES (?, ?, ?, ?, ?, ?, ?)',
    ).run(
      pluginId,
      '课堂实时投票与抽奖转盘',
