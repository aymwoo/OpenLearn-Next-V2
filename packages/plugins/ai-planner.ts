...
export function bootstrapAIPlannerPlugins() {
  const { commandBus, actionRegistry, db, processManager, eventBus } = kernelContainer;

  // 1. Setup the long-running task handler for generating content/schedules
  processManager.registerHandler('ai_planner_task', async (processId, payload, state, log, updateState) => {
  ...
  commandBus.registerHandler('ai.start_generation', {
  ...
  commandBus.registerHandler('ai.apply_recommendation', {
    async execute(command) {
      const payload = command.payload as any;
      const id = uuidv7();
  ...
  commandBus.registerHandler('ai.apply_grade', {
    async execute(command) {
      const payload = command.payload as any;
      db.prepare(`
